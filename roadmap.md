...
## Phase 86 — Real CV file import (PDF/DOCX/TXT) بالاستخراج المحلي (مكتملة)
- `src/lib/cv-extract.ts` جديد: استخراج النص داخل المتصفح فقط — TXT عبر `File.text()`، PDF عبر `pdfjs-dist` (worker عبر `?url` ديناميكي)، DOCX عبر `mammoth/mammoth.browser.js`. لا رفع للملف الأصلي إلى Storage أو الخادم إطلاقاً، ولا تنفيذ لأي ماكرو (قراءة طبقة النص فقط).
- حدود موثّقة في `CV_FILE_LIMITS`: 5MB، 40 صفحة PDF، حد أدنى 50 حرفاً، حد أقصى 20,000 حرف للتحليل.
- أخطاء صديقة AR/EN: TOO_LARGE, UNSUPPORTED_TYPE, LEGACY_DOC (‎.doc غير مدعوم صراحةً)، PDF_ENCRYPTED, PDF_TOO_MANY_PAGES, CORRUPT, TOO_SHORT — وفشل الاستخراج يحافظ على النص السابق في المربع.
- تجاوز 20k: لا قصّ صامت — إشعار + toast يوضّح الطول الأصلي وأن أول 20,000 حرف فقط عُرضت للمراجعة والاختصار.
- تطبيع: إزالة المحارف الصفرية/التحكم، توحيد الأسطر، الحفاظ على العربية والإنجليزية.
- الواجهة (`cv-import.tsx`): طريقتان (رفع ملف / لصق النص)، dropzone كـ`<label>` + `<input type=file accept=".pdf,.docx,.txt,...">` (منتقي ملفات عادي على الجوال، سحب وإفلات اختياري على سطح المكتب)، اسم الملف مقصوص بلا تجاوز + الحجم + شريط تقدم + زر إزالة 44px، حالة الاستخراج عبر `aria-live="polite"`، والنص المستخرج يملأ المربع القابل للتحرير قبل التحليل.
- الخصوصية: «الملف نفسه لا يُحفظ؛ نستخرج النص في جهازك…» أعلى الحقل + إشعار Phase85 موسّع أعلى زر التحليل (الاستخراج محلي، والنص فقط يُرسل عند اختيار التحليل، والتعبئة اليدوية بديل دائم).
- الحفظ: تدفق المراجعة قبل الحفظ كما هو؛ عند عدم مطابقة التخصص تلقائياً يظهر «التخصص لم يُطابق تلقائياً — اختره يدوياً» ويبقى المستخدم موجّهاً لإكمال الملف (Phase80 تفرض الاكتمال خادمياً). لا توثيق تلقائي لأي ترخيص.
- الأداء: استيراد ديناميكي لـpdfjs/mammoth داخل معالج الملف فقط — لا تدخل حزمة الصفحات العامة/الرئيسية/المصادقة.
- لا `console.*` ولا تقرير أخطاء يحتوي محتوى الملف.
- اختبارات متصفح فعلية عند 390px: TXT عربي/إنجليزي، PDF صفحتان، DOCX، 25k حرف => تحذير بدل فقد صامت، ‎.doc مرفوض، 6MB مرفوض، PDF تالف => رسالة صديقة مع بقاء النص. tsgo نظيف + build OK.

## Phase 87 — بحث الفرص العامة: تصفية وتقسيم صفحات على الخادم (مكتملة)
- دالتان SECURITY INVOKER تقرآن من `public_jobs`/`public_shifts` المنقّحة فقط: `search_public_jobs` (q ≤80 حرفاً على العنوان/المدينة/الدولة/اسم التخصص AR+EN، country، city، specialty_id، specialty_ids، employment_type، exclude_ids للوظائف المقدَّم عليها) و`search_public_shifts` (نفس المعاملات + الملاحظات العامة، بلا نوع توظيف). كلتاهما تعيدان `total_count` عبر `count(*) over()`، وحجم الصفحة محصور خادمياً `least(greatest(limit,1),50)`، وترتيب حتمي: وظائف درجة المطابقة ثم created_at desc ثم id؛ مناوبات درجة المطابقة ثم starts_at asc ثم id. تنفيذ مسموح لـanon/authenticated/service_role فقط بعد revoke all.
- `public_listing_places()` تعيد الدول/المدن المتاحة بدل اشتقاقها من صفوف محمّلة.
- `sort=match` يُحسب في القاعدة (تطابق التخصص 2 + الدولة 1) للمسجّلين فقط، و`scope=mine/field` يتحول إلى قائمة تخصصات مسموحة تُمرَّر كتصفية لا كتفويض، و`hideApplied` يمرّر معرفات تقديمات المستخدم نفسه فقط.
- الواجهة: `useInfiniteQuery` بمفاتيح تشمل كل الفلاتر والترتيب وحجم الصفحة، إلغاء الطلبات القديمة عبر `AbortSignal`، تأخير 300ms لكلمة البحث مع تنفيذ فوري عند Enter، 20 نتيجة للتبويب المفرد و10 لكل مصدر في التبويب المخلوط (مناوبات قادمة ثم أحدث الوظائف)، زر «تحميل المزيد» بارتفاع 44px مع «عرض X من Y» وإعلان عدد المضاف عبر aria-live، وأي تغيير فلتر يعيد ضبط الترقيم مع بقاء كل شيء في الرابط.
- الاختبار: ترقيم فعلي على القاعدة (offsets 0/1/2/3 بصفوف مختلفة وtotal_count=3، بحث بلا نتائج = صفر صفوف، حد 999 يُقصّ)؛ متصفح على 320/390/1440 لكل من /jobs والبحث والتصفية بالدولة وتبويب المناوبات: لا تجاوز أفقي ولا أخطاء وحدة تحكم. البناء وفحص الأنواع نظيفان.

## Phase 88 — اتساق لغة اسم المنشأة + دقة صياغة الحجز/التقديم (مكتملة)
- `facilityDisplayName(row, lang)` في `src/lib/format.ts`: بالعربية نفضّل `name_ar` ثم `name_en`، وبالإنجليزية `name_en` ثم `name_ar`، وعند غياب الاثنين تسمية محايدة (منشأة / Employer). لا يغيّر أي اسم مخزّن، والأسماء القديمة العربية فقط تظهر كما هي في الإنجليزية بدل الفراغ.
- طُبّق حيث تُكشف الهوية فعلياً: تفاصيل الوظيفة، تفاصيل المناوبة، ملف المنشأة العام، الدعوات، ترويسة المحادثة، بطاقات طلبات التقديم، واسم الحساب في مركز الحساب (مع إضافة `name_en` إلى الاستعلامات التي كانت تجلب `name_ar` فقط، ومفتاح اللغة في مفتاح التخزين المؤقت للطلبات).
- صياغة المناوبة: «اسم المنشأة محجوب حتى تحجز المناوبة أو يبدأ التواصل معك» و«الحجز مجاني للكوادر الصحية. بعد نجاح الحجز يمكن للمنشأة التواصل معك للتنسيق حول التفاصيل.» — لم يعد أي نص يلمّح إلى موافقة ثانية من المنشأة بعد أن يُنشئ `book_open_shift` حجزاً مؤكداً فوراً. نافذة التأكيد تبقى تذكّر بالالتزام بالحضور.
- صياغة الوظيفة: كشف الهوية صار مطابقاً لـ`can_view_facility_identity` — تقدّم علاقة التوظيف (فرز/مقابلة/عرض) أو تواصل المنشأة أو دعوتها، بدل «بعد قبول طلبك» فقط، في النصين العربي والإنجليزي وفي نص الحالة المكشوفة.
- الاختبار: اختبار وحدة للمساعد (اسمان معاً بالعربية والإنجليزية، إنجليزي فارغ يرجع للعربي، بلا أسماء يرجع للمحايد) = 5/5؛ متصفح على 320/390/1440 بالعربية والإنجليزية لصفحة وظيفة عامة: النص الجديد ظاهر بكل لغة، لا تجاوز أفقي ولا أخطاء وحدة تحكم. (لا توجد حالياً مناوبة عامة ولا اسم منشأة إنجليزي في البيانات الحقيقية، فلم تُنشأ بيانات وهمية.) البناء وفحص الأنواع نظيفان.

## Phase 89 — بيانات وصفية ديناميكية لصفحات الفرص العامة بلا تسريب هوية (مكتملة)
- `src/lib/seo.ts`: `fetchPublicJobMeta` / `fetchPublicShiftMeta` تقرأان من العرضين المنقّحين `public_jobs` / `public_shifts` فقط بالمفتاح العام (لا جداول أصلية ولا أي حقل منشأة/ناشر/تواصل)، مع `jobSeoText` / `shiftSeoText` لعنوان ووصف مختصرين.
- تفاصيل الوظيفة: عنوان `<المسمى> — <المدينة، الدولة> | SyndeoCare`، ووصف من المسمى + التخصص + الموقع + نوع التوظيف + نطاق الراتب، ونفس النص في og وtwitter، والرابط الأساسي دائماً رابط الـslug حتى عند فتح الصفحة بالمعرّف.
- تفاصيل المناوبة: نفس النمط مع وقت البدء والأجر بالساعة.
- الفرص المغلقة أو غير العامة: وسوم محايدة + `noindex,nofollow` + رابط أساسي إلى /jobs — لا يتسرّب عنوان أو تفاصيل الإعلان القديم في ترويسة الخادم.
- لم تُضَف بيانات `JobPosting` المنظّمة لأن Google يشترط `hiringOrganization` حقيقية بينما إخفاء هوية المنشأة قرار منتج؛ اختلاق ناشر كان سيعطي بيانات مضللة. وكذلك لا مخطط للمناوبات.
- صفحات التخصصات والمدونة والأدلة وأسئلة المقابلات تملك عناوين ووصفاً خاصاً بها أصلاً، فلم تُغيَّر. روابط `/jobs?...` المفلترة تبقى أساسيتها /jobs.
- الاختبار: ترويسة الخادم للوظائف الثلاث العامة تعطي عناوين فريدة تحوي المسمى والموقع دون أي اسم منشأة أو معرّف مستخدم أو بيانات تواصل؛ فتح الوظيفة بالمعرّف يعطي رابطاً أساسياً بالـslug؛ رابط غير موجود ومناوبة غير عامة يعطيان وسوم noindex؛ خريطة الموقع سليمة؛ البناء وفحص الأنواع نظيفان.

## Phase90 — طلبات تعديل البيانات الموثّقة بأنواع صحيحة (مكتملة)
- `LockedField` صار يفصل `currentStoredValue` (القيمة المعتمدة في القاعدة) عن `currentDisplayValue` (التسمية البشرية)، ويقبل `editor`: نص بحد أقصى، رقم بمدى، تخصص، دولة، مدينة تابعة للدولة، نوع منشأة.
- التخصص كان يُرسل كاسم مترجم بينما القاعدة تتوقع UUID — الآن قائمة بحث تعرض الاسم وتخزّن المعرّف. الدول ونوع المنشأة كذلك تخزّن القيمة المعتمدة لا التسمية.
- تحقق قبل الإرسال: قيمة غير فارغة، مختلفة عن الحالية، رقم صحيح 0..60، أطوال النصوص، والاختيار من القائمة فقط.
- عرض الطلبات المعلّقة وقائمة «طلبات تعديل بياناتي» وطابور المشرف تعرض تسميات مفهومة، ومع التخصص يظهر المعرّف الخام كنص تقني ثانوي.
- المرفق: `accept` صار PDF/JPG/PNG/WEBP فقط مع بيان حد 10 ميغابايت (لا يعرض ملفات DOC غير المدعومة).
- القاعدة تبقى المرجع: `review_change_request` صارت تتحقق من نوع المنشأة ضمن المجموعة المعتمدة، ومن أطوال الاسم/الترخيص/المدينة/الدولة/الجوال، ومن UUID التخصص، ومن سنوات الخبرة 0..60، وتُخزّن القيمة بعد التشذيب.
- فحص البيانات: لا توجد أي طلبات تعديل قائمة (الجدول فارغ)، فلا سجلات قديمة تحتاج معالجة أو رفضاً.
- البناء وفحص الأنواع نظيفان.

## Phase91 — الحقول النظامية (المعرّفات والتواريخ وحالة النظام) مملوكة للخادم (مكتملة)
- الرسائل: أُلغي منح الإدراج العام واستُبدل بأعمدة المحتوى فقط (`conversation_id, sender_id, body, attachment_path/name/type/size`). المعرّف ووقت الإنشاء من افتراضات القاعدة، و`read_at`/`delivered_at` يفرضها `guard_message_insert_receipts` كـNULL عند الإدراج لأي دور غير موثوق — فلا يستطيع عميل معدّل إرسال رسالة «مقروءة/مسلّمة» مسبقاً. إيصالات Phase73 (التحديث) كما هي، وservice_role يحتفظ بكامل الصلاحية.
- تنبيهات الوظائف: الإدراج على `user_id, specialty_id, country, city, employment_type, channel, whatsapp_phone, is_active` فقط، والتحديث على التفضيلات فقط. `guard_job_alert_system_state` يعيد فرض `id/user_id/created_at/last_sent_at` من الصف القديم لأي دور غير موثوق، فيبقى `last_sent_at` بيد مُرسل التنبيهات (service_role) ولا تتشوّه نوافذ الإرسال/إعادة المحاولة.
- الملفات الشخصية: الإدراج على `id, full_name, phone, country, city, avatar_url` (المعرّف مطلوب لملكية auth.uid) والتحديث على الحقول الشخصية فقط؛ `created_at/updated_at` عبر الافتراضات والمشغّل.
- الوظائف المحفوظة: أُلغي التحديث كلياً (التجربة إضافة/حذف فقط)، والإدراج على `job_id, user_id` فقط.
...
## Phase 93 — تثبيت شروط المناوبة بعد الحجز وتوثيق تعديلات الوظائف (مكتملة)
- المناوبات: مشغّل `guard_shift_terms_locked` (BEFORE UPDATE) يرفض تغيير `title, notes, specialty_id, starts_at, ends_at, hourly_rate, currency, country, city, facility_id` متى كانت الحالة السابقة `booked/completed/cancelled` بخطأ `SHIFT_TERMS_LOCKED`. تبقى انتقالات النظام (`status, booked_by, applications_count, updated_at, facility_verified`) عبر `cancel_facility_shift`/`complete_shift`/`book_open_shift`، وإلغاء الكادر يعيد فتح المناوبة بنفس شروطها دون تعديل.
- الواجهة: لا يوجد إجراء تعديل للمناوبة أصلاً؛ أُضيف بيان «تم تثبيت تفاصيل المناوبة بعد الحجز / Shift terms are locked after booking» على بطاقة أي مناوبة غير مفتوحة، مع بقاء الإلغاء والإتمام بعد الانتهاء والسجل للقراءة فقط.
- الوظائف: أُلغي التحديث المباشر لأعمدة المحتوى من authenticated ولم يبقَ سوى `is_active` (الإغلاق/إعادة النشر). كل تعديل لمحتوى الوظيفة يمر عبر `public.update_job_listing(...)` (SECURITY DEFINER + `require_mfa` + ملكية المنشأة + قفل الصف) مع تحقق من العنوان/الوصف/نطاق الراتب/الخبرة/الشواغر، ومنع الشواغر تحت عدد الموظّفين (`VACANCIES_BELOW_HIRED`) ومنع تاريخ انتهاء ماضٍ لوظيفة نشطة (`EXPIRY_IN_PAST`).
- سجل التعديلات: جدول `public.job_change_events` (الحقول المتغيّرة + فرق JSON بلا أي حقول هوية + عدد من أُشعروا) بـRLS: يقرأه المالك والمشرف والمتقدّم لتلك الوظيفة فقط، ومنح SELECT على الأعمدة الآمنة فقط.
- الإشعارات: عند وجود متقدّمين يصل لكل متقدّم لم ينسحب إشعار واحد «تم تحديث تفاصيل وظيفة تقدمت لها / A job you applied to was updated» بربط `/activity?tab=applications`، ويعرض «طلباتي» سطراً خفيفاً بتاريخ التحديث بعد التقديم. الطلب يبقى سارياً.
- لم يُجرَ أي تعبئة رجعية: الوظائف الخمس ذات الطلبات والمناوبات الأربع المحجوزة بقيت كما هي (سجل التعديلات فارغ).
- الاختبار (بدور authenticated وبيانات QA ملغاة بالكامل بعد الفحص): تعديل أجر مناوبة محجوزة => `SHIFT_TERMS_LOCKED`؛ تعديل عنوان وظيفة مباشرة => permission denied؛ `is_active` يعمل؛ تعديل وظيفة بلا متقدمين => نجح بسجل تعديل واحد و0 إشعارات؛ تعديل وظيفة لها متقدّم => سجل واحد (`title+salary`) وإشعار واحد لكل متقدّم؛ تاريخ انتهاء ماضٍ مرفوض؛ غير المالك => `FORBIDDEN`. لم تبقَ أي بيانات QA.
- إصلاح عاجل ضمن نفس المرحلة: تعديل سابق ألغى كل صلاحيات `authenticated` على `facilities` و`healthcare_professionals` دون إعادة منحها، فكانت كل قراءات الحسابات المسجّلة تفشل بـ«permission denied». أُعيد منح SELECT مع أعمدة الإنشاء/التعديل الخاصة بالملف فقط.

## Phase 94 — سلامة مرفقات المحادثة بين القاعدة والتخزين وتنظيف الملفات اليتيمة (مكتملة)
- `validate_message_insert` صارت SECURITY DEFINER (search_path = public, storage) وتتحقق عند وجود مرفق: المسار يبدأ بمعرّف المحادثة، والكائن موجود فعلاً في `chat-attachments` (وإلا `ATTACHMENT_NOT_FOUND`)، ورافعه هو المُرسِل (وإلا `ATTACHMENT_NOT_OWNED`)، ومجلده هو المحادثة نفسها. الثقة تُشتق من الطلب (`auth.uid()` / دور `service_role`) لا من `current_user`، لأن SECURITY DEFINER يجعل المنفّذ هو المالك.
- التخزين هو المرجع: النوع يُشتق من `split_part(mimetype,';',1)` والحجم من بيانات التخزين ويُكتبان فوق ما يرسله العميل، مع التحقق من القائمة المسموحة و10 ميغابايت. اسم العرض يُشذَّب (بلا أسطر ولا `/`، 255 حرفاً) ويُقبل فقط إذا طابق امتداده امتداد الملف المخزّن، وإلا يُستبدل باسم الملف الفعلي — فلا يغيّر الاسم أو النوع دلالات العرض.
- توحيد القديم: حُدِّثت الرسائل الصوتية الأربع من `audio/webm;codecs=opus` إلى `audio/webm` مطابقةً للتخزين (الأحجام مطابقة أصلاً ولم تُمس أي بيانات أخرى).
- العميل: `uploadChatFile` يقبل مساراً معروفاً مسبقاً، وأُضيفت `removeChatFile` لحذف الملف من التخزين عند فشل حفظ الرسالة أو فشل/إلغاء الرفع بعد اكتماله — والحذف بأفضل جهد ولا يحجب الخطأ الأصلي، ولا يُحذف الملف عند نجاح الإرسال.
- العرض: `attachmentKind(path, type)` يسمح بالمعاينة المدمجة فقط عندما يتطابق الامتداد مع نوع MIME ضمن قائمة الصور/الصوت/الفيديو (تبقى الرسائل الصوتية `.webm` تعمل)، وأي شيء آخر — بما فيه HTML/SVG — يُفتح كرابط تنزيل.
- تنظيف: أُزيل الملف اليتيم الوحيد (صورة PNG قديمة 145 بايت) عبر واجهة التخزين بعد إعادة التأكد من عدم ارتباطه بأي رسالة — لا حذف عبر SQL من `storage.objects` ولا حذف جماعي.
- صيانة: `public.admin_chat_attachment_orphans()` (مشرف + تحقق ثنائي) تعرض فقط الملفات الأقدم من 24 ساعة بلا رسالة مرتبطة، دون أي حذف تلقائي تفادياً لسباق مع إرسال جارٍ.
- الاختبار (بدور authenticated): ملف المستخدم في محادثته => نجح مع نوع/حجم/اسم مأخوذة من التخزين (تجاهل `text/html` و`evil.exe`)؛ ملف رفعه الطرف الآخر => `ATTACHMENT_NOT_OWNED`؛ مسار غير موجود => `ATTACHMENT_NOT_FOUND`؛ ملف من محادثة أخرى => `INVALID_ATTACHMENT_PATH`؛ رسالة نصية عادية => تعمل. بعد التنظيف: 0 ملف يتيم و0 تعارض بيانات. البناء وفحص الأنواع نظيفان.

## Phase 95 — لا تُسجَّل حالة «مكتمل» لطلب حذف الحساب بلا حذف فعلي (مكتملة)
- آلة الحالات الموثّقة: `pending` → `processing` أو `rejected` فقط من لوحة الإدارة (مشرف + تحقق ثنائي)، و`processing` حالة نهائية بالنسبة للوحة بانتظار المنفّذ الموثوق. أي محاولة لتسجيل `completed` ترفع `DELETION_FINALIZER_REQUIRED` مع رسالة مفهومة.
- `completed` بقيت في المخطط للتوافق المستقبلي لكن لا يملك أي مسار مصادق صلاحية ضبطها؛ تُحجز لمنفّذ `service_role` مستقبلي بعد حذف فعلي مُتحقَّق منه.
- الواجهة: حُذف زر «تمت المعالجة» نهائياً، وصار الطلب قيد المعالجة يعرض «لا يتم اعتبار الحساب محذوفاً حتى تُنفَّذ عملية الحذف/إخفاء الهوية الموثوقة»، مع قائمة تشغيلية غير قابلة للنقر تبيّن ما يجب أن يغطيه الحذف الموثوق (حساب الدخول، الملفات الخاصة، إيقاف الظهور، إخفاء الهوية مع الإبقاء على سجل التعاملات، عدم المساس بسجلات الطرف الآخر) — بلا أي توجيه لحذف صفوف يدوياً.
- صفحة «الحساب والخصوصية» للمستخدم تقول صراحةً إن الحساب ما زال موجوداً ويمكن الدخول إليه حتى تُنفَّذ العملية؛ لا نص يزعم أن الحساب حُذف. نص سياسة الخصوصية (الطلب يُراجَع قبل تنفيذه) بقي كما هو ولا وعد بحذف فوري ذاتي.
- تقرير جاهزية الإطلاق: `false_completed_deletions` حاجب إذا وُجد طلب مكتمل وحساب صاحبه ما زال موجوداً، و`stale_deletion_processing` تنبيه لأي طلب بقي قيد المعالجة أكثر من 7 أيام (ليس حاجباً).
- وثيقة تصميم فقط: `docs/account-deletion-finalizer.md` تصف متطلبات المنفّذ المستقبلي دون تنفيذ أي حذف مُتلف في هذه المرحلة.
- الاختبار (بدور مشرف بتحقق ثنائي على طلب QA مؤقت أُلغي بالكامل): `pending` → `processing` نجح؛ `completed` => `DELETION_FINALIZER_REQUIRED`؛ `processing` → `rejected` => `INVALID_DELETION_TRANSITION`. عدد الطلبات في النظام 0، والفحصان الجديدان يعطيان 0. البناء وفحص الأنواع نظيفان.

## Phase 96 — البحث المجهول عن المرشحين لا يكشف معرّف حساب الدخول (مكتملة)
- نتائج `search_candidates_idempotent` لم تعد تتضمن `user_id` إطلاقاً: تعيد معرّف الملف المهني (candidate_id) مع الحقول المجهّلة فقط (التخصص، سنوات الخبرة، الدولة/المدينة، الاستعداد للمناوبات، حالة التوثيق).
- الذاكرة المؤقتة `candidate_search_requests.result` صارت تُخزَّن بلا `user_id`، ونُظّفت النتائج المحفوظة سابقاً (10 صفوف → 0 صف يحتوي معرّف حساب). `candidate_search_access` ما زال يحفظ معرّف المختص داخلياً على الخادم فقط ولا يُعاد للعميل.
- مسارات جديدة موثوقة: `start_candidate_conversation_from_search` و`send_candidate_invitation_from_search` تستقبلان candidate_id وتحلّانه على الخادم عبر `private.resolve_searchable_candidate` (تشترط الظهور والموافقة الصريحة وحساب دخول حيّ)، ثم تمرّان على نفس فحوص المنشأة والتوثيق والاشتراك وصلاحية البحث. الدالتان القديمتان بقيتا لتدفقات العلاقة القائمة فقط (متقدمون، حجوزات، ملف مرشح بعد التواصل).
- الواجهة: نوع المرشح لم يعد يحمل user_id، وزر التواصل وزر الدعوة في نتائج البحث يمرّران candidate_id، ولا يُبنى أي رابط قبل التواصل يحتوي معرّف حساب. صفحة ملف المرشح تبقى محكومة بالعلاقة كما هي.
- الصلاحيات (Phase60): REVOKE ثم GRANT EXECUTE صريح للدوال العامة الثلاث لـauthenticated وservice_role، ودالة الحل الداخلية لـservice_role فقط.
- الاختبار (بجلسة منشأة بتحقق ثنائي داخل معاملة أُلغيت بالكامل): النتيجة لا تحتوي مفتاح user_id، المعرّف المُعاد هو معرّف الملف المهني، الطلب المكرر يعيد النتائج ذاتها، التواصل عبر candidate_id نجح، معرّف غير معروف أو مرشح أوقف ظهوره => `CANDIDATE_NO_LONGER_SEARCHABLE`. لم تتغير أي بيانات حقيقية (منشآت موثّقة 0، محادثات 12 كما هي). البناء وفحص الأنواع نظيفان.

## Phase 97 — وصول الزائر للبيانات بقائمة معتمدة فقط (مكتملة)
- تثبيت الإصلاح المباشر في هجرة متتبّعة وقابلة لإعادة التنفيذ: سحب كل صلاحيات `anon` عن الجداول الخاصة (الطلبات، طلبات البحث، رسائل التواصل، المحادثات، الوثائق والاعتمادات، المقابلات، الدعوات، التنبيهات، التفاعلات، الإشعارات، سجلات وطلبات تعديل الملف، الملفات الشخصية، التقييمات، المحفوظات، الحجوزات، الأدوار وغيرها).
- قائمة القراءة المعتمدة للزائر أصبحت ثلاثة كائنات فقط: `public_jobs`، `public_shifts`، `specialties` — وقراءة فقط.
- قائمة تشغيل الدوال للزائر أصبحت نقاط القراءة العامة الموثّقة فقط: `search_public_jobs`، `search_public_shifts`، `public_listing_places`. سُحبت صلاحية التشغيل العامة عن الدوال المساعدة والحارسة (`slugify`، `is_known_city_country_valid`، حُرّاس التنبيهات والرسائل وشروط المناوبات) ومُنحت للمسجّلين/الخدمة فقط. عدد دوال SECURITY DEFINER القابلة للتشغيل من الزائر = 0.
- `release_privilege_audit` صار يتضمن فحوصاً مانعة جديدة: `anon_table_allowlist`، `anon_allowlist_read_only`، `anon_function_allowlist`، ورُفع `public_security_definer_functions` إلى مانع. نتيجة التدقيق الحالية: كل الفحوص ok(0).
- وثيقة `docs/anon-access-allowlist.md`: سياسة القائمة المعتمدة، جدول الفحوص المتوقعة، وإجراء إضافة أي مورد عام مستقبلي عبر view/RPC منقّح مع توثيق الاستثناء — لا إعادة صلاحية لجدول أساسي.
- اختبارات الانحدار (زائر غير مسجّل): الرئيسية و/jobs و/shifts و/specialties و/blog و/guides و/contact و/auth تحمّل بنجاح بلا أخطاء وحدة تحكم؛ صفحة تفاصيل الوظيفة تعمل على 320/390/1440 بلا تجاوز أفقي وتبقي ناشر الوظيفة محجوباً. نموذج التواصل أُرسل فعلياً ووصل عبر دالة الخادم بصلاحية الخدمة (لا يحتاج قراءة الزائر)، ثم حُذفت رسالة الاختبار. القراءة المباشرة لـ`profiles` كزائر مرفوضة، وإدراج تنبيه وظيفة/حفظ وظيفة كمستخدم مسجّل ما زال يعمل (اختبار أُلغي بالكامل).
- ملاحظة: `/auth/register` و`/auth/reset` غير موجودتين كمسارات أصلاً (التسجيل على `/register` واستعادة كلمة المرور على `/reset-password`) — ليست انحداراً من هذه المرحلة.

## Phase 98 — دوال المشغّلات داخلية فقط (مكتملة)
- هجرة متتبّعة وقابلة لإعادة التنفيذ تسحب كل صلاحيات التشغيل عن دوال المشغّلات في المخطط العام من PUBLIC و`anon` و`authenticated` وتُبقيها لحساب الخدمة فقط — شاملة الدوال العشر المرصودة (`guard_profile_ownership`، `normalize_document_input`، `normalize_facility_input`، `normalize_job_alert_input`، `normalize_job_input`، `normalize_professional_input`، `normalize_profiles_input`، `normalize_shift_input`، `normalize_trusted_device_input`، `validate_known_location`) وكذلك `guard_job_alert_system_state` و`guard_message_insert_receipts` و`guard_shift_terms_locked` التي كانت ما زالت متاحة للمسجّلين.
- `normalize_trusted_device_input` ما زال مرتبطاً بمشغّل فعّال على `trusted_devices`، فلم يُحذف — اكتُفي بسحب الصلاحية.
- لم تُمسّ الدوال العادية غير المشغّلة (`slugify` وغيرها) — صلاحياتها كما أُقرّت في المرحلة 97.
- `release_privilege_audit`: فحص مانع جديد `client_executable_trigger_functions` (أي دالة مشغّل قابلة للاستدعاء من PUBLIC/زائر/مسجّل = حاجب). القيمة الحالية 0.
- اختبار انحدار (بيانات اختبار حُذفت فوراً): إدراج تنبيه وظيفة كمستخدم مسجّل نجح والتطبيع اشتغل عبر المشغّل (`  yemen  ` => `yemen`)، بينما الاستدعاء المباشر لدالة المشغّل أرجع «رفض صلاحية» قبل أي خطأ سياق مشغّل. جميع مشغّلات التطبيع والتحقق من الموقع ما زالت مرتبطة بجداول الكادر والمنشآت والوظائف والمناوبات والوثائق والتنبيهات والملفات والأجهزة الموثوقة.
- البناء وفحص الأنواع نظيفان. تنبيهات الفاحص الـ58 السابقة دون تغيير.

## Phase 99 — معنى موحّد لانتهاء موعد التقديم (مكتملة)
- قاعدة مشتركة في قاعدة البيانات `job_accepting_applications(is_active, expires_at)`: الوظيفة تستقبل طلبات فقط إذا كانت مفتوحة ولم ينتهِ موعد التقديم. لم تُستخدم داخل العرض العام حتى لا يحتاج الزائر صلاحية تشغيل دالة (التزاماً بقائمة المرحلة 97)، والعرض العام يحتفظ بالشرط نفسه مكتوباً مباشرة.
- حصة الباقة: `enforce_plan_limits` صار يحسب الوظائف المستقبِلة للطلبات فقط — الوظيفة المنتهي موعدها لا تستهلك حصة نشر بعد اليوم. القفل المتزامن لكل منشأة كما هو.
- سدّ ثغرة التمديد/إعادة الفتح: مشغّل جديد `jobs_enforce_limits_on_reopen` يفحص الحصة عند أي تحديث ينقل الوظيفة من «غير مستقبِلة» إلى «مستقبِلة» (تمديد الموعد أو إعادة النشر)، ضمن المعاملة نفسها؛ عند امتلاء الحصة يُرفض التعديل ويبقى الموعد القديم كما هو.
- الدعوات: `private.invitation_target_available` تستخدم القاعدة الموحدة — لا إرسال ولا قبول دعوة وظيفة بعد الموعد (منطق القبول من المرحلة 74 كما هو، والدعوة المعلّقة تظهر «غير متاحة» بحكم الساعة دون تغيير حالتها).
- لوحة المنشأة: عدّاد «الوظائف النشطة» يعتمد القاعدة نفسها، والوظيفة المنتهي موعدها تظهر بشارة «انتهى موعد التقديم / Application deadline passed» بدل «منشورة»، ويختفي إجراء الدعوة لها — بينما يظل فتح المتقدمين وجدولة المقابلات ومسار التوظيف متاحاً كالمعتاد.
- شاشة طلبات المختص: زر «إعادة التقديم» بعد السحب يختفي إذا انتهى موعد التقديم.
- SEO/الاكتشاف العام: الوظائف المنتهية خارج العرض العام وخريطة الموقع وصفحتها العامة تعرض حالة «مغلقة» كما أُقرّ سابقاً.
- اختبار انحدار كامل ببيانات QA أُنشئت وحُذفت بالكامل بعده (0 صفوف متبقية، ولم تُمسّ أي بيانات حقيقية): النشر ضمن الحصة نجح، نشر ثانٍ عند امتلاء الحصة = JOB_QUOTA_EXCEEDED، بعد انتهاء الموعد اختفت الوظيفة من العرض العام وتحرّرت الحصة ونجح نشر وظيفة جديدة، تمديد الموعد مع حصة ممتلئة = JOB_QUOTA_EXCEEDED مع بقاء الموعد القديم، وبعد إغلاق الوظيفة الأخرى نجح التمديد وعادت الوظيفة للعرض العام، والوظيفة المغلقة يدوياً بموعد مستقبلي بقيت خارج العرض العام.
- فحص الأنواع والبناء نظيفان. تنبيهات الفاحص الـ58 السابقة دون تغيير.

## Phase 100 — التحقق بخطوتين إلزامي لحسابات الإدارة (مكتملة)
- دالتان جديدتان: `admin_mfa_access_ok()` (إداري + عامل TOTP موثّق + جلسة aal2) و`require_admin_mfa()` بأخطاء صريحة: NOT_ADMIN / ADMIN_MFA_ENROLLMENT_REQUIRED / MFA_REQUIRED. الصلاحيات لـauthenticated وservice_role فقط.
- الـ14 دالة الإدارية (مراجعة الوثائق ومستندات المنشآت، التوثيق، طلبات التغيير، الأدوار، بلاغات السلامة، طلبات حذف الحساب، تقارير الجاهزية والصلاحيات، تقارير السلامة والصيانة) تستخدم `require_admin_mfa()` بدل `require_mfa()` العام.
- الثغرة المغلقة: `mfa_access_ok()` تعيد true لمن لا يملك عاملاً موثّقاً (مقصود للمستخدم العادي)، فكان المدير الذي يزيل عامله يواصل العمليات الحساسة على aal1. لم يعد ممكناً.
- RLS: 16 سياسة إدارية + 5 سياسات قراءة فيها فرع إداري تحوّلت إلى `admin_mfa_access_ok()`، وكذلك سياسات التخزين لدلوَي `facility-docs` و`credentials` — لا قراءة وثائق لمدير بلا عامل أو على aal1.
- جاهزية الإطلاق: مانع جديد `admin_without_mfa` إذا وُجد حساب إداري بلا عامل TOTP موثّق.
- الواجهة: صفحة الأمان تُظهر لحسابات الإدارة «التحقق بخطوتين مطلوب لحسابات الإدارة» ولا تعرض زر الإيقاف (الخادم هو المرجع). لوحة الإدارة لمدير بلا عامل تعرض بوابة تفعيل تربط صفحة الأمان ولا تحمّل أي طابور حسّاس، ومدير بعامل على aal1 يُحوَّل إلى `/mfa-challenge?next=/admin` كما هو.
- المستخدم العادي دون تغيير: التحقق بخطوتين يبقى اختيارياً، وإن فُعّل تبقى قاعدة aal2 للبيانات الخاصة.
- اختبار ببيانات QA حُذفت فوراً: مدير aal2 يعمل، مدير aal1 = MFA_REQUIRED، مدير بلا عامل = ADMIN_MFA_ENROLLMENT_REQUIRED مع حجب RLS والتخزين (0 صفوف)، مستخدم عادي = NOT_ADMIN دون تأثر، والجاهزية ترفع المانع.
- فحص الأنواع والبناء نظيفان.

## Phase 101 — إشعارات الرسائل لا تكرّر نص المحادثة الخاصة (مكتملة)
- `notify_new_message()` كانت تنسخ أول 120 حرفاً من نص الرسالة إلى جسم الإشعار. صارت تنشئ إشعاراً عاماً فقط: «رسالة جديدة / New message» بلا جسم، ورابط إلى `/messages` — لا مقتطف نص، ولا اسم مرفق، ولا موضوع المحادثة.
- migration متتبّعة وقابلة لإعادة التشغيل تطابق الدالة الحيّة، وتمسح أي نص محفوظ في إشعارات الرسائل القديمة، وتمنع استدعاء الدالة مباشرة (EXECUTE مسحوب من PUBLIC/anon/authenticated، service_role فقط) — تعمل عبر مشغّل إدراج الرسائل حصراً.
- الواجهة: بطاقة الإشعار في الجرس وصفحة الإشعارات تعرض الجسم شرطياً أصلاً، فلا فراغ ولا سطر خالٍ عند غياب النص. عدّاد الرسائل غير المقروءة يبقى مستقلاً كما هو موثّق للمستخدم.
- القنوات الخارجية: مذكّرة خصوصية صريحة في `notify.server.ts` بأن البريد/واتساب/الدفع لا يعيد استخدام نص المحادثة بدون قرار خصوصية/منتج صريح.
- اختبار ببيانات QA أُنشئت وحُذفت بالكامل (0 صفوف متبقية): رسالتان في الاتجاهين ولّدتا إشعارين عامّين — واحد لكل مستلم، لا إشعار ذاتي للمرسل، `with_body=0`، لا تسريب لأي نص من الرسالة في أي إشعار، والرابط `/messages`، وعدّاد غير المقروء للمستلم سليم. رسائل الصوت/الصورة تمرّ بالمسار نفسه والدالة لا تقرأ حقول النص أو المرفق إطلاقاً.
- إجمالي إشعارات الرسائل في القاعدة: 10، منها 0 تحتوي نصاً.
- فحص الأنواع والبناء نظيفان.

## Phase 102 — مرجع واحد للدول عبر التسجيل والملفات والبحث
- ملف جديد `src/lib/countries.ts` هو المصدر الوحيد: 18 دولة، كل واحدة `{ code, stored, ar, en, aliases }`، مع `canonicalCountry` و`countryDisplay` و`countryEntry` و`STORED_COUNTRIES` و`DEFAULT_COUNTRY`. التخزين يبقى بالاسم العربي المعتمد (لا انتقال إلى ISO في هذا الإصدار).
- حُذفت القوائم المكرّرة: `COUNTRIES`/`COUNTRY_EN`/`ISO_COUNTRY` من `format.ts` و`CODE_BY_STORED`/`DEFAULT_COUNTRY` من `geo.ts` — `format.countryLabel` و`geo.countryOptions` و`cityOptions` كلها تشتقّ من المرجع الواحد.
- أسماء الدول داخل بيانات المدن في `geo.ts` صارت القيم المعتمدة نفسها (السعودية/الإمارات/عُمان) فلا يمكن أن يتباعد التسجيل عن التعديل.
- تسجيل صاحب العمل يحوّل الاختيار إلى `canonicalCountry` قبل الحفظ؛ فلاتر البحث العام وبحث المرشحين تطبّع المرادف القادم من رابط قديم إلى القيمة المعتمدة فتبقى الروابط المحفوظة تعمل.
- migration: `public.canonical_country(text)` (IMMUTABLE، EXECUTE مسحوب من PUBLIC/anon، ممنوح لauthenticated/service_role) + تطبيع idempotent لجميع القيم المعروفة في profiles / healthcare_professionals (بما فيها license_country) / facilities / jobs / shifts / job_alerts. النصوص غير المعروفة تُحفظ كما هي (trim فقط) ولا تُحذف.
...
## Phase 104 — سد فجوات وثيقة المتطلبات (قيد التنفيذ)
- [x] Migration: إحداثيات الكادر/المنشأة، التوفر، الأجر المرغوب، `push_subscriptions`، `expiry_alert_log`، `distance_km`، توسيع `refresh_verification_expiry` بتنبيهات 30/14/0 يوماً، `rehire_shift` الذرّية.
- [ ] واجهة الموقع والنطاق (ملف الكادر + ملف المنشأة + بحث المرشحين بالمسافة).
- [ ] التوفر والأجر المرغوب + بطاقة الأرباح في نشاط الكادر.
- [ ] زر إعادة التوظيف في حجوزات المنشأة.
- [ ] إشعارات Push للمتصفح + مفتاح تفعيل في الإعدادات.
- [ ] إعداد بريد notify.syndeocare.ai (طلب المستخدم 2026-09-20).
