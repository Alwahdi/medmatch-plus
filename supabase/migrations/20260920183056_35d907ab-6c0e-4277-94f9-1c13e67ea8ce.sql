-- Phase 97: anonymous DB access allowlist only (tracked copy of applied hotfix)
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    'credentials','facilities','facility_documents','facility_subscriptions',
    'healthcare_professionals','interviews','invitations','job_alerts','job_change_events',
    'jobs','message_reactions','messages','notifications','profile_change_log',
    'profile_change_requests','profiles','reviews','safety_reports','saved_jobs',
    'shift_bookings','shifts','subscription_plans','trusted_devices','user_roles'
  ]
  LOOP
...
-- Explicit anon SELECT allowlist (only these three)
GRANT SELECT ON public.public_jobs TO anon;
GRANT SELECT ON public.public_shifts TO anon;
GRANT SELECT ON public.specialties TO anon;

-- Anon EXECUTE allowlist: documented public read endpoints only
REVOKE EXECUTE ON FUNCTION public.guard_job_alert_system_state() FROM anon;
REVOKE EXECUTE ON FUNCTION public.guard_message_insert_receipts() FROM anon;
REVOKE EXECUTE ON FUNCTION public.guard_shift_terms_locked() FROM anon;
REVOKE EXECUTE ON FUNCTION public.is_allowed_upload(text, text, jsonb) FROM anon;
...
BEGIN
  ...
  -- Phase 97: anonymous table/view access must be allowlisted, not merely write-free.
  RETURN QUERY
  ...
  -- Phase 97: allowlisted anon objects must stay read-only.
  RETURN QUERY
  ...
  -- Phase 97: anon EXECUTE limited to documented public read endpoints.
  RETURN QUERY
  ...
  FROM (
    ...
    WHERE a.privilege_type = 'EXECUTE'
      ...
      AND p.proname NOT IN ('search_public_jobs','search_public_shifts','public_listing_places','is_known_city_country_valid')
  ) s;
