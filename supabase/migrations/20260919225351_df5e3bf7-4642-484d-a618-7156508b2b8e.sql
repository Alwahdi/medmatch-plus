...
GRANT EXECUTE ON FUNCTION public.can_read_avatar_path(text, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_read_avatar_path(text, uuid) TO service_role;

DROP POLICY IF EXISTS "avatars read all" ON storage.objects;
DROP POLICY IF EXISTS "avatars read authorized" ON storage.objects;
CREATE POLICY "avatars read authorized"
ON storage.objects
FOR SELECT
TO authenticated
USING (
  bucket_id = 'avatars'
  AND public.can_read_avatar_path((storage.foldername(name))[1], auth.uid())
);