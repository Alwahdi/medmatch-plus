CREATE OR REPLACE FUNCTION public.admin_update_account_deletion(_request_id uuid, _status text, _note text DEFAULT NULL::text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 'completed' is reserved for a trusted service-role finalizer that actually removes
  -- the auth account and anonymises personal data. No admin client may claim it.
  IF _status = 'completed' THEN
    RAISE EXCEPTION 'DELETION_FINALIZER_REQUIRED';
  END IF;
  IF _status NOT IN ('processing','rejected') THEN
  ...
  -- state machine: pending -> processing | rejected. 'processing' stays terminal for the
  -- admin client until the trusted finalizer exists.
  IF NOT (_current = 'pending' AND _status IN ('processing','rejected')) THEN
  ...
  UPDATE public.account_deletion_requests
     SET status = _status,
         ...
         processed_at = CASE WHEN _status = 'rejected' THEN now() ELSE processed_at END
...
BEGIN
  ...
  -- Deletion requests: a request may only be 'completed' after real deletion/anonymisation.
  RETURN QUERY
  SELECT 'false_completed_deletions'::text, CASE WHEN c = 0 THEN 'ok' ELSE 'blocker' END, c::int,
         'Deletion requests marked completed while the account still exists (status would be untrue).'
  FROM (
    SELECT count(*) c FROM public.account_deletion_requests r
    JOIN auth.users u ON u.id = r.user_id
    WHERE r.status = 'completed'
  ) s;

  RETURN QUERY
  SELECT 'stale_deletion_processing'::text, CASE WHEN c = 0 THEN 'ok' ELSE 'warning' END, c::int,
         'Deletion requests left in processing for more than 7 days without a trusted finalizer run.'
  FROM (
    SELECT count(*) c FROM public.account_deletion_requests
    WHERE status = 'processing' AND updated_at < now() - interval '7 days'
  ) s;
