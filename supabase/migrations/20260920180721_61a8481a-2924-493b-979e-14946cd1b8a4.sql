-- ===== A) Booked/finished shift terms are immutable =====
CREATE OR REPLACE FUNCTION public.guard_shift_terms_locked()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
AS $$
BEGIN
  IF OLD.status IN ('booked','completed','cancelled') AND (
    ...
    RAISE EXCEPTION 'SHIFT_TERMS_LOCKED';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_guard_shift_terms_locked ON public.shifts;
CREATE TRIGGER trg_guard_shift_terms_locked
BEFORE UPDATE ON public.shifts
FOR EACH ROW EXECUTE FUNCTION public.guard_shift_terms_locked();

-- ===== B) Audit table for material job edits =====
CREATE TABLE IF NOT EXISTS public.job_change_events (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  job_id uuid NOT NULL REFERENCES public.jobs(id) ON DELETE CASCADE,
  changed_by uuid,
  changed_fields text[] NOT NULL,
  diff jsonb NOT NULL DEFAULT '{}'::jsonb,
  applicants_notified integer NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS job_change_events_job_idx ON public.job_change_events(job_id, created_at DESC);

REVOKE ALL ON public.job_change_events FROM PUBLIC, anon, authenticated;
GRANT SELECT (id, job_id, changed_fields, created_at) ON public.job_change_events TO authenticated;
GRANT ALL ON public.job_change_events TO service_role;

ALTER TABLE public.job_change_events ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "job change events readable by owner, applicant, admin" ON public.job_change_events;
CREATE POLICY "job change events readable by owner, applicant, admin"
ON public.job_change_events FOR SELECT TO authenticated
USING (
  EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'admin'::public.app_role)
  OR EXISTS (SELECT 1 FROM public.jobs j JOIN public.facilities f ON f.id = j.facility_id
             WHERE j.id = job_change_events.job_id AND f.user_id = auth.uid())
  OR EXISTS (SELECT 1 FROM public.applications a
             WHERE a.job_id = job_change_events.job_id AND a.user_id = auth.uid())
);

-- ===== C) Trusted job content update =====
...
BEGIN
  ...
  -- collect the material diff (no publisher/owner fields)
  IF btrim(_title) IS DISTINCT FROM _job.title THEN
    _changed := _changed || 'title'; _diff := _diff || jsonb_build_object('title', jsonb_build_object('old', _job.title, 'new', btrim(_title)));
  END IF;
  IF btrim(_description) IS DISTINCT FROM _job.description THEN _changed := _changed || 'description'; END IF;
  IF _specialty_id IS DISTINCT FROM _job.specialty_id THEN
    _changed := _changed || 'specialty_id'; _diff := _diff || jsonb_build_object('specialty_id', jsonb_build_object('old', _job.specialty_id, 'new', _specialty_id));
  END IF;
  IF _employment_type IS DISTINCT FROM _job.employment_type THEN
    _changed := _changed || 'employment_type'; _diff := _diff || jsonb_build_object('employment_type', jsonb_build_object('old', _job.employment_type, 'new', _employment_type));
  END IF;
  IF _country IS DISTINCT FROM _job.country THEN
    _changed := _changed || 'country'; _diff := _diff || jsonb_build_object('country', jsonb_build_object('old', _job.country, 'new', _country));
  END IF;
  IF _city IS DISTINCT FROM _job.city THEN
    _changed := _changed || 'city'; _diff := _diff || jsonb_build_object('city', jsonb_build_object('old', _job.city, 'new', _city));
  END IF;
  ...
  IF _min_experience IS DISTINCT FROM _job.min_experience THEN
    _changed := _changed || 'min_experience'; _diff := _diff || jsonb_build_object('min_experience', jsonb_build_object('old', _job.min_experience, 'new', _min_experience));
  END IF;
  IF COALESCE(btrim(_required_license),'') IS DISTINCT FROM COALESCE(_job.required_license,'') THEN
    _changed := _changed || 'required_license';
  END IF;
  IF _vacancies IS DISTINCT FROM _job.vacancies THEN
    _changed := _changed || 'vacancies'; _diff := _diff || jsonb_build_object('vacancies', jsonb_build_object('old', _job.vacancies, 'new', _vacancies));
  END IF;
  IF _expires_at IS DISTINCT FROM _job.expires_at THEN
    _changed := _changed || 'expires_at'; _diff := _diff || jsonb_build_object('expires_at', jsonb_build_object('old', _job.expires_at, 'new', _expires_at));
  END IF;

  IF array_length(_changed, 1) IS NULL THEN
    RETURN _job_id; -- nothing material changed
  END IF;
...
-- ===== D) Listing content is RPC-only for clients; is_active stays a direct owner action =====
